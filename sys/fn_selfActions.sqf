...
if (_canDo && (speed player <= 1) && _hasToolbox) then {
	if (s_player_deploybike < 0) then {
		s_player_deploybike = player addaction[("<t color=""#007ab7"">" + ("Deploy Bike (will use Toolbox)") +"</t>"),"external\bike.sqf","",5,false,true,"", ""];
	};
} else {
...
call compile preprocessFileLineNumbers 'external\mf-tow\init.sqf';
